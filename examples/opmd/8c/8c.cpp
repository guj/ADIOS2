...
// C++ 14, but adios is only in C++11
//using MaxResSteadyClock = std::conditional_t<std::chrono::high_resolution_clock::is_steady,
//                                            std::chrono::high_resolution_clock,
//                                            std::chrono::steady_clock>;
// Using steady clock directly
using MaxResSteadyClock = std::chrono::steady_clock;
...
public:
  ...
  [[nodiscard]] Extent ProperExtent (unsigned long long n, bool isJoinedDim,  bool init) const;

  void setAttribute(std::string const &key, std::string const& val)
  {
    m_WriterIO.DefineAttribute<std::string>(key, val);
  }

  void setAttribute(std::string const &key, int& val)
  {
    m_WriterIO.DefineAttribute<std::int64_t>(key, val, "", "", true);
  }
  ...
  std::string m_StrCharge = "/data/particles/ion/charge";
  std::string m_StrID     = "/data/particles/ion/id";
  std::string m_StrPosX   = "/data/particles/ion/position/x";
private:  
};
...
Extent ADIOSWriteSeries::ProperExtent (unsigned long long n, bool useJoinedDim, bool init) const
{
  if (!useJoinedDim)
   ...
   if (init)
     return {adios2::JoinedDim};
   else
     return {};
}


void ADIOSWriteSeries::setupCurrScope(bool useJoinedDim, unsigned long np, unsigned long offset, unsigned long count)
{
  auto var_charge =  m_WriterIO.InquireVariable(m_StrCharge);
  auto var_id = m_WriterIO.InquireVariable(m_StrID);
  auto var_posx = m_WriterIO.InquireVariable(m_StrPosX);

  //Extent shape = ProperExtent(np, useJoinedDim, true);
  if (!var_charge) // found
    {
      m_WriterIO.DefineVariable <double>   (m_StrCharge, ProperExtent(np, useJoinedDim, true), ProperExtent(offset, useJoinedDim, false), {count});
      m_WriterIO.DefineVariable <double>   (m_StrPosX,   ProperExtent(np, useJoinedDim, true), ProperExtent(offset, useJoinedDim, false), {count});
      m_WriterIO.DefineVariable <uint64_t> (m_StrID,     ProperExtent(np, useJoinedDim, true), ProperExtent(offset, useJoinedDim, false), {count});
      return;
    }

  var_charge.SetShape(ProperExtent(np, useJoinedDim, true));
  var_charge.SetSelection({ ProperExtent(offset, useJoinedDim, false), {count} });
  
  var_posx.SetShape(ProperExtent(np, useJoinedDim, true));
  var_posx.SetSelection({ ProperExtent(offset, useJoinedDim, false), {count} });
  
  var_id.SetShape(ProperExtent(np, useJoinedDim, true));
  var_id.SetSelection({ ProperExtent(offset, useJoinedDim, false), {count} });

}

ADIOSWriteSeries::~ADIOSWriteSeries()
{
  m_Writer.Close();
}

/*
void ADIOSWriteSeries::WritePtl()
{
  auto var_charge = m_WriterIO.InquireVariable(m_StrCharge);
  auto var_posx = m_WriterIO.InquireVariable(m_StrPosX);
  auto var_id = m_WriterIO.InquireVariable(m_StrID);
...
public:
    ...
    void run();
    void store(ADIOSWriteSeries &series, int step);
    void storeParticles(ADIOSWriteSeries &curr, int &step);
...
void parse(TestInput &input, std::string line)
{
    ...
    if (vec[0].compare("ptlUnit") == 0)
    {
      input.m_PtlUnit = (unsigned long) atoi(vec[1].c_str()) ;
    }

    if (vec[0].compare("maxMil") == 0)
    {
        input.m_PtlMax = ( (unsigned long) atoi(vec[1].c_str()) ) * (unsigned long) input.m_PtlUnit;
        return;
    }

    if (vec[0].compare("minMil") == 0)
    {
        input.m_PtlMin = ( (unsigned long) atoi(vec[1].c_str()) ) * (unsigned long) input.m_PtlUnit;
...
int main(int argc, char *argv[])
{
...

    {
      ...
      {
#else
       std::map<std::string, LocalProfiler>::iterator it;
       for (it = m_GlobalProfilers.begin(); it != m_GlobalProfilers.end(); it++)
       {
	 auto name = it->first;
	 auto p = it->second;
#endif
...
void BasicParticlePattern::run()
{
    { // group/var based
...

        {
            ...
            ADIOSWriteSeries series(filename, MPI_COMM_WORLD);
...
/*
 * Write a ADIOSWriteSeries
 *
 * @param ADIOSWriteSeries   input
 * @param step     iteration step
 *
 */
void BasicParticlePattern::store(ADIOSWriteSeries &series, int step)
{
  /*
 ...
 *
 */
void BasicParticlePattern::storeParticles(ADIOSWriteSeries &curr, int &step)
{
    Checkpoint g("StorePtls", m_Input.m_MPIRank);

    curr.setAttribute("particleSmoothing", "none");
    curr.setAttribute("openPMD_STEP", step);
    ...
    curr.setupCurrScope(m_Input.m_UseJoinedDim, np, offset, count);
...
void BasicParticlePattern::printMe()
{
  ...
  std::cout << "\t  Num steps: "<<m_Input.m_Steps
	    << "\n\t  NumPtls per rank/step: "<<m_Input.m_PtlMin/m_Input.m_PtlUnit<<"  to "<<m_Input.m_PtlMax/m_Input.m_PtlUnit<<" Units,  size of unit: "<<m_Input.m_PtlUnit
            << std::endl;
} // printMe
